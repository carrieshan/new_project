...
def main():
    # 获取用户输入的数据库路径
    db_path = input("请输入要检查的数据库文件路径（按回车使用示例数据库）: ").strip()
    ...
    if not db_path or not os.path.exists(db_path):
        ...
        db_path = sample_db
        print(f"使用示例数据库: {db_path}")
    
    # 执行数据库检查
    checker = DatabaseChecker(db_path)
    
    # 获取要检查的表列表
    tables_input = input("请输入要检查的表名（用逗号分隔，按回车检查所有表）: ").strip()
    tables_to_check = [t.strip() for t in tables_input.split(',')] if tables_input else None
    
    checker.run_all_checks(tables_to_check)
